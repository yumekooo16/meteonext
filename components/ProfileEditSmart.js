...
export default function ProfileEditSmart() {
  ...
  const [tableName, setTableName] = useState(null); // 'profiles' ou 'users'
  ...
  // Fonction pour détecter quelle table utiliser
  const detectTable = async () => {
    try {
      // Essayer d'abord profiles
      ...
      if (!usersError) {
        console.log('✅ Table users détectée');
        return 'users';
      }

      console.log('❌ Aucune table détectée');
      return null;
    } catch (error) {
      console.error('Erreur détection table:', error);
      return null;
    }
  };
  ...
  const loadUserData = async () => {
    try {
      ...
      // Détecter quelle table utiliser
      const detectedTable = await detectTable();
      setTableName(detectedTable);

      if (!detectedTable) {
        setError('Aucune table de profil trouvée. Contactez l\'administrateur.');
        return;
      }

      // Récupérer les données du profil depuis la table détectée
      ...
      if (profileError && profileError.code !== 'PGRST116') {
        console.error(`Erreur récupération profil (${detectedTable}):`, profileError);
      }
  ...
  const updateUserProfile = async () => {
    try {
      ...
      if (!tableName) {
        throw new Error('Table de profil non détectée');
      }

      // Mise à jour du nom dans la table détectée
      if (formData.name !== originalData.name) {
        console.log(`🔄 Mise à jour du nom dans ${tableName}...`);
        ...
        if (nameError) {
          console.error(`Erreur mise à jour nom (${tableName}):`, nameError);
          throw new Error(`Erreur lors de la mise à jour du nom: ${nameError.message}`);
        }
        console.log(`✅ Nom mis à jour dans ${tableName}`);
      }
      ...
      <div className="max-w-md mx-auto bg-white rounded-lg shadow-md p-6">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">
            Aucune table de profil trouvée dans la base de données.
          </p>
          <button
  ...
  return (
    <div className="max-w-md mx-auto bg-white rounded-lg shadow-md p-6">
      ...
      {/* Indicateur de table utilisée */}
      <div className="mb-4 p-2 bg-blue-50 rounded-md">
        <p className="text-xs text-blue-600">
          Table utilisée: <strong>{tableName}</strong>
        </p>
      </div>
      ...
      <div className="mt-6 p-4 bg-blue-50 rounded-md">
        ...
        <ul className="text-xs text-blue-700 space-y-1">
          ...
          <li>• Table utilisée: {tableName}</li>
        </ul>
      </div>
    </div>
  );
